...
import org.zerock.service.WffLabReplyService;

import lombok.AllArgsConstructor;
import lombok.extern.log4j.Log4j;

@RequestMapping("/wfflabreplies/")
@RestController
@Log4j
@AllArgsConstructor 
//ㄴ>굳이 setter 주입 안해도 된다 이거 주석처리하면 @Setter(onMethod_ = @Autowired) 근데 우리는 AllArgsConstructo이걸로 작업
public class WffLabReplyController {
	
	private WffLabReplyService service; //서비스 필드 선언
	private WffLabMapper boardmapper;
	
	//댓글 등록create()
	@PostMapping(value = "/new", consumes = "application/json", produces = {MediaType.TEXT_PLAIN_VALUE})
	public ResponseEntity<String> create(@RequestBody WffLabReplyVO vo){
	...
	public ResponseEntity<WffLabReplyPageDTO> getList( //ReplyPageDTO로 객체타입을 바꾸면 List형태로 담을 필요없다.
	...
	public ResponseEntity<WffLabReplyVO> get(
			@PathVariable("rno") Long rno){
		log.info("댓글 상세 조회 내용 확인 ! ==>" + rno);
		return new ResponseEntity<WffLabReplyVO>(service.get(rno), HttpStatus.OK);
	}
	...
	public WffLabMapper getBoardmapper() {
		return boardmapper;
	}



	public void setBoardmapper(WffLabMapper boardmapper) {
