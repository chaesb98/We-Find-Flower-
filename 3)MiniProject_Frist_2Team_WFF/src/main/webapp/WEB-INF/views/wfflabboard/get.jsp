...
<%@include file="../includes/header.jsp" %> 
	 ...
	 <!--get CSS-->
	 <link rel="stylesheet" href="/resources/wfflabboard/get.css">
        ...
        <div class = "panel-body">
          ...
          <div class="form-group">
					<label>제목</label> <input class="form-control" name="title"
						value='<c:out value="${wfflabboard.title}"/>' readonly="readonly">
				</div>
				<div class="form-group">
					<label>내용</label>
					<textarea rows="5" class="form-control" name="content"
						readonly="readonly"><c:out
							value="${wfflabboard.content}" /> </textarea>
				</div>
				<div class="form-group">
				...
				<button data-oper='modify' class="sub_btn btn-default"
					onclick="location.href='/wfflabboard/modify?bno=<c:out value="${wfflabboard.bno}"/>'">수정</button>
				<button data-oper='list' class="reset_btn btn-default"
					onclick="location.href='/wfflabboard/list'">글 목록</button>

				<!-- Page317 : 기존 get.jsp 에서 버튼을 클릭하면 form 태그를 이용하는 방식이기에
					     데이터를 추가해서 이동하도록 수정 코딩해 줍니다. -->
				<form id='operForm' action="/wfflabboard/modify" method="get">
						<input type="hidden" id="bno" name="bno" value='<c:out value="${wfflabboard.bno}"/>'>
...
<!--========== javascript ============-->
<script type="text/javascript" src="/resources/js/wfflabreply.js"></script>
...
<script type="text/javascript">
	...
	var bnoValue = '<c:out value="${wfflabboard.bno}"/>';
	var replyUL = $(".chat");
	...
	function showList(page) {
		...
		wfflabreply.getList({bno:bnoValue,page: page||1}, function (replyCnt, list) {
			...
			for (var i = 0, len = list.length || 0; i < len; i++) {
				...
				str += "<small class='pull-right text-muted'>"
						+ wfflabreply.displayTime(list[i].replyDate)
						+ "</small></div>";
	...
	replyPageFooter.on("click", "li a", function (e) {
		...
		//모달 (댓글)등록 및 갱신 'Reply' 'Replyer' 이벤트
 		modalRegisterBtn.on("click", function(e) {
			...
			wfflabreply.add(reply, function(result) {
				alert(result);
		...
		//모달(댓글)조회 Ajax처리 수정과 삭제가 필요한 댓글 번호(rno) 'data-rno' 속성 만들어서 추가
 		$(".chat").on("click","li", function (e) {
 			...
 			wfflabreply.get(rno, function (reply) {
 				modalInputReply.val(reply.reply);
 				modalInputReplyer.val(reply.replyer);
 				modalInputReplyDate.val(wfflabreply.displayTime(reply.replyDate)).attr("readonly","readonly");
		...
		//모달(댓글) 수정, 페이징 목록
 		modalModBtn.on("click", function (e) {
 			...
 			wfflabreply.update(reply, function (result) {
 				alert(result);
 				modal.modal("hide");
		...
		//모달(댓글) 삭제
 		modalRemoveBtn.on("click", function (e) {
 			...
 			wfflabreply.remove(rno, function (result) {
 				alert(result);
 				modal.modal("hide");
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("button[data-oper='modify']").on("click", function(e) {
			operForm.attr("action", "/wfflabboard/modify").submit();
		});
		
		$("button[data-oper='list']").on("click", function(e) {
			operForm.find("#bno").remove();
			operForm.attr("action", "/wfflabboard/list")
			operForm.submit();
		});
	});
</script>
