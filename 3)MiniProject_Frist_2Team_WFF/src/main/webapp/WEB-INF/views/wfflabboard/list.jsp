...
<%@include file="../includes/header.jsp" %> 
	 ...
	 <!--list CSS-->
	 <link rel="stylesheet" href="/resources/wfflabboard/list.css">
	...
	<div class="col-lg-12">
		...
		<div class="panel-body">
          ...
          <p>Community</p>
	            	...
	            	<c:forEach items="${list}" var="wfflabboard">
						<tr>
							<td><c:out value="${wfflabboard.bno}"></c:out></td>
							...
							<td><a class="move" href='<c:out value="${wfflabboard.bno}"/>'>
							<c:out value="${wfflabboard.title}" /> <b>[ <c:out value="${wfflabboard.replyCnt}" />  ]</b>
								</a>
							</td>
														
							<td><c:out value="${wfflabboard.writer}"></c:out></td>
							<td><fmt:formatDate value="${wfflabboard.regdate}"
									pattern="yyyy-MM-dd" />
							<td><fmt:formatDate value="${wfflabboard.updateDate}"
									pattern="yyyy-MM-dd" />
						</tr>
				...
				<form id="actionForm" action="/wfflabboard/list" method="get">
				...
				<div class="row">
					<div class="col-lg-12">
						<form action="/wfflabboard/list" id="searchForm" method="get">
							<select name="type">
								<option value=""
							...
							<button id="regBtn" type="button" name="id" data-oper='list' class="btn btn-xs pull-right" onclick="location.href='/wfflabboard/list'">글등록하기(Register)</button>
					</div>						
				</div>
		</div><!-- /.panel -->
	</div><!-- /.col-lg-12 -->
...
<script type="text/javascript">
	...
	// list.jsp에서  Register New Board 버튼 클릭하면
		...
		$("#regBtn").on("click", function() {
			self.location = "/wfflabboard/register";
	});
		...
		$(".move").on("click", function(e) {
			...
			actionForm.attr("action", "/wfflabboard/get");
			actionForm.submit();
		});
